...
Auto-updated tracking of AMD GPU ecosystem projects. Last updated: **2026-07-29 04:49 UTC**
...
an error instead of reporting a successful sync with stale evidence.
`UPSTREAM_COMMENT_TOKEN` remains separate and is required only to update the
pinned upstream umbrella comment; without it, Ready Tickets still refreshes the
dashboard snapshots in read-only mode.
